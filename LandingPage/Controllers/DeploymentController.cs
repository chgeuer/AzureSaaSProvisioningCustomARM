namespace LandingPage.Controllers
{
    ...
    public class DeploymentController : ControllerBase
    {
        ...
        {
            var templateInformation = MyExtensions.Deserialize<TemplateInformation<SampleTemplateParametrization>>(token, cfg.ApiKey);
            var templateUrl = Url2.Combine(templateInformation.BaseAddress, filename); // https://flurl.dev/docs/fluent-url/
            var unpatchedContent = await httpClient.GetStringAsync(templateUrl);
            var patcher = templateInformation.Parametrization.GetContentPatcher();
